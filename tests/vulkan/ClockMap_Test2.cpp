...
void main ()
{
	...
	imageStore( un_Output, ivec2(gl_LaunchIDEXT), payload );
		...
		CHECK_ERR( vulkan.Compile( OUT rayGenShader, {rt_shader, raygen_shader_source}, EShLangRayGen, ETraceMode::TimeMap, 1 ));
	}
...
void main ()
{
	payload = vec4( 0.412f, 0.796f, 1.0f, 1.0f );
		...
		CHECK_ERR( vulkan.Compile( OUT rayMissShader, {rt_shader, raymiss_shader_source}, EShLangMiss ));
	}
...
void main ()
{
	...
	payload = vec4(barycentrics, 1.0);
		...
		CHECK_ERR( vulkan.Compile( OUT rayClosestHitShader, {rt_shader, closesthit_shader_source}, EShLangClosestHit, ETraceMode::TimeMap, 1 ));
	}
	return true;
}
